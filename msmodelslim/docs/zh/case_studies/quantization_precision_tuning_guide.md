...
4. **进阶方案**：若调整后精度仍无法满足需求，可进一步尝试启用Flex Smooth Quant算法，或通过叠加QuaRot算法实现协同优化。INT4/w4a8可以使用Flex AWQ SSZ算法。
...
5. **AutoRound**：高精度上限。适用于对精度极度敏感的场景，虽然运行较慢，但量化效果最接近浮点。
...
4. **PDMIX**：平衡策略。旨在平衡推理过程中的激活量化精度与计算性能。
